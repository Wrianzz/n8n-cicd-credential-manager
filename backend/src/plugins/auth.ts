...
function forbiddenHtml(requiredRole: string) {
  return `
<!doctype html>
<html>
  <head>
  ...
  <body>
    <div class="card">
      <h1>Access denied</h1>
      <p>
        Login berhasil, tapi akun kamu belum punya client role
        <code>${requiredRole}</code>
        untuk mengakses Credential Control Panel.
      </p>
      <p>
        Hubungi admin Keycloak untuk assign role tersebut ke akun kamu.
      </p>
...
function getClientRoles(claims: Record<string, any>, clientId: string): string[] {
  const roles = claims.resource_access?.[clientId]?.roles

  return Array.isArray(roles) ? roles.map(String) : []
}

function hasRequiredClientRole(
  accessClaims: Record<string, any>,
  idClaims: Record<string, any>
): boolean {
  const requiredRole = config.OIDC_REQUIRED_CLIENT_ROLE
  ...
  const accessTokenRoles = getClientRoles(accessClaims, clientId)
  const idTokenRoles = getClientRoles(idClaims, clientId)
...
export async function registerAuth(app: FastifyInstance) {
  ...
  app.get('/api/auth/callback', async (req, reply) => {
    ...
    if (!hasRequiredClientRole(accessClaims, idClaims)) {
      const requiredRole = config.OIDC_REQUIRED_CLIENT_ROLE || 'required-role'
      ...
      req.log.warn({
        ...
        resourceAccess: accessClaims.resource_access ?? idClaims.resource_access
      }, 'user does not have required client role')
    
      return reply
        .code(403)
        .type('text/html')
        .send(forbiddenHtml(requiredRole))
    }
