...
export function validateCredentialMap(input: {
  ...
  for (const entry of schema.data.entries) {
    ...
    const effectiveKeys = credentialKeys.length > 1 && 
      typeof (node.parameters as any)?.genericAuthType === 'string' && 
      credentialKeys.includes((node.parameters as any).genericAuthType)
        ? [(node.parameters as any).genericAuthType]
        : credentialKeys;
      
    // Ganti pengecekan di bawahnya menggunakan variabel baru 'effectiveKeys'
    ...
    results.push(issue({
      ...
      meta: {
        inferredCredentialKey: effectiveKeys[0],
        nodeType: node.type
      }
    }))
  }
